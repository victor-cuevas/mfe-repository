...
import { CreditProviderDto } from '../models/depot-product.model';
import { DepotProductService } from '../service/depot-product.service';

@Injectable({
  providedIn: 'root'
})
export class CreditProviderResolver implements Resolve<CreditProviderDto> {
  constructor(private service: DepotProductService, public spinnerService: SpinnerService) { }

  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> {
    this.spinnerService.setIsLoading(true);
    return this.service.getCreditProvider().pipe(
      catchError((error) => {
