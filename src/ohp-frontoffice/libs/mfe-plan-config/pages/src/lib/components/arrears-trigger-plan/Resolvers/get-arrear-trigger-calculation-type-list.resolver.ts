...
export class GetArrearTriggerCalculationTypeListResolver implements Resolve<boolean> {
  constructor(public arrearTriggerService: ArrearsTriggerPlanService, public spinnerService: SpinnerService) { }

  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> {
    return this.arrearTriggerService.getArrearTriggerCalculationTypeList().pipe(
      catchError((error) => {
