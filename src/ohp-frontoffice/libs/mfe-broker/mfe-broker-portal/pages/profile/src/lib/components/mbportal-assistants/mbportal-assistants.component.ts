...
import { getPortalUser, loadPortalUserFailure, loadPortalUserSuccess } from '@close-front-office/mfe-broker/mfe-broker-portal/core';
import {
  IntermediaryRole,
...
@Component({
  templateUrl: './mbportal-assistants.component.html',
})
export class MbportalAssistantsComponent implements OnInit, OnDestroy {
  ...
  assistants$ = this.store.select(getPortalUser).pipe(
    takeUntil(this.onDestroy$),
    map(portalUser => JSON.parse(JSON.stringify(portalUser?.subordinateIntermediaries || []))),
  );
    ...
    this.portalUserService
      ...
      .subscribe(() => {
        this.portalUserService
          ...
          .subscribe(
            response => {
              return this.store.dispatch(loadPortalUserSuccess({ entity: response }));
            },
            (error: any) => {
              return this.store.dispatch(loadPortalUserFailure({ error }));
            },
          );
      });
  }
