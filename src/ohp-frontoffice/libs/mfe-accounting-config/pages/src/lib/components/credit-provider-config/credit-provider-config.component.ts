...
@Component({
  selector: 'mactc-credit-provider-config',
  templateUrl: './credit-provider-config.component.html',
  styleUrls: ['./credit-provider-config.component.scss']
})
export class CreditProviderConfigComponent implements OnInit {
  ...
  placeholder = 'select';
  Response: CreditProviderConfigForAccountingModuleDto[] = [];
  financialAmortizationTypeResponse: FinancialAmortizationTypeDto[] = [];
  creditProviderNameResponse: CreditProviderNameDto[] = [];
  index: any;
  isClicked!: boolean;
  ...
  Nothide!: boolean;
  isErrors!: boolean;
  section!: CreditProviderConfigForAccountingModuleDto;
  selectedDetails!: CreditProviderConfigForAccountingModuleDto
  deletedRecords: CreditProviderConfigForAccountingModuleDto[] = [];
  ...
  navigateUrl!: string
  actionDeselect: CreditProviderNameDto = { ...new CreditProviderNameDto() };
  dataSelected: any;
  Header = this.translate.instant('accounting.Validation.Header');
  editable!: boolean;
  errorCode!:string
  creditHeader = [
    { header: this.translate.instant('accounting.CreditProvider.tabel.CreditProvider'), field: 'creditProviderName.caption', width: '92%' },
    { header: this.translate.instant('accounting.CreditProvider.tabel.delete'), field: 'delete', width: '8%', fieldType: 'deleteButton' }];
  constructor(
    ...
    public actionService: CreditProviderService,
    public fluidValidation: fluidValidationService
  ) {
    this.editable = true;
    this.businessError = this.translate.instant('accounting.CreditProvider.ValidationError.buisnessError');
    this.businessErrorforFinancialAmortizationType = this.translate.instant('accounting.CreditProvider.ValidationError.businessErrorforFinancialAmortizationType');
  ...
  ngOnInit(): void {
    ...
    this.route.data.subscribe(
      (res: any) => {
        if (res.creditProviderList != null) {
          this.spinnerService.setIsLoading(false);
          this.creditProviderNameResponse = [...(res.creditProviderList as CreditProviderNameDto[])];
        }
      }
    );
    this.route.data.subscribe(
    ...
    this.route.data.subscribe(
      (res: any) => {
        if (res.creditProviderData != null) {
          this.spinnerService.setIsLoading(false);
          this.Response = (res.creditProviderData as CreditProviderConfigForAccountingModuleDto[]);
          this.DropdownConfig.externalError = false;
          this.index = 0;
          this.deletedRecords = [...new Array<CreditProviderConfigForAccountingModuleDto>()];
  ...
  click(data: CreditProviderConfigForAccountingModuleDto) {
    if (data) {
      const filteredData = this.Response.map(item => {
        return item.creditProviderName?.caption;
      });
  ...
  onRowDelete(dataselect: CreditProviderConfigForAccountingModuleDto, array: CreditProviderConfigForAccountingModuleDto[]) {
    ...
    if (
      ((dataselect.creditProviderName?.caption == null) &&
      ...
      if (!this.isErrors || (this.isErrors && dataselect.isEntered)) {
        ...
        if (!hasValue || (hasValue && dataselect.isEntered)) {
          ...
          this.section = new CreditProviderConfigForAccountingModuleDto();
          if (this.Response.length > 0) {
            this.section.creditProviderName =
              this.Response[this.Response.length - 1].creditProviderName
  ...
  addRow() {
    ...
    if (!this.userDetailsform.invalid || this.Response.length == 0) {
      if (!hasValue) {
        if (!this.isErrors) {
          this.SettingFalse();
          const addRow = new CreditProviderConfigForAccountingModuleDto();
          ...
          const codetable = { ...new CreditProviderNameDto() };
          addRow.creditProviderName = codetable;
  ...
  changeSectionGridData(event: any) {
    if (event != undefined) {
      ...
      if (event?.value != null) {
        this.Response[this.index].creditProviderName = event?.value;
        this.section.creditProviderName = event?.value;
        this.DropdownConfig.externalError = false;
        this.removeBusinessErrorCall();
      } else {
        this.Response[this.index].creditProviderName = null;
        this.section.creditProviderName = null;
        this.DropdownConfig.externalError = true;
      }
    }
  }
  ...
  onYes(GridData: CreditProviderConfigForAccountingModuleDto[]) {
    this.show = false;
  ...
  onSave(GridData: CreditProviderConfigForAccountingModuleDto[]) {
    ...
    if (hasValue) {
      ...
      saveData.forEach(x => {
        ...
        this.deletedRecords.forEach(y => {
          if (y.state == DtoState.Deleted && y.creditProviderName?.caption == null) {
            y.creditProviderName = null;
          }
        });
      });
      ...
      this.actionService.saveCreditProviderScreenData(this.deletedRecords).subscribe(
        res => {
          this.spinnerService.setIsLoading(false);
          this.editable = true;
          this.deletedRecords = [...new Array<CreditProviderConfigForAccountingModuleDto>()];

          this.actionService.getCreditProviderScreenData().subscribe(res => {
            this.spinnerService.setIsLoading(false);
            this.financialAmortizationTypeList = [];
            const getResponse = res as CreditProviderConfigForAccountingModuleDto[]
            ...
            if (this.Response.length > 0) {
              ...
              this.index = this.Response.findIndex(section => {
                return (
                  section.creditProviderName?.caption == this.section.creditProviderName?.caption 
                );
              });
  ...
  buildConfiguration() {
    const creditProviderRequired = new ErrorDto();
    creditProviderRequired.validation = 'required';
    creditProviderRequired.isModelError = true;
    creditProviderRequired.validationMessage =
      this.translate.instant('accounting.CreditProvider.ValidationError.section') +
      this.translate.instant('accounting.CreditProvider.ValidationError.required');
    this.DropdownConfig.Errors = [creditProviderRequired];
