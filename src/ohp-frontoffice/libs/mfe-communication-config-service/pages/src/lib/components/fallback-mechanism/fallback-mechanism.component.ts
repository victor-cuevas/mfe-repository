...
import { FallbackCommunicationDto } from './Models/fallback-communicationDto.model';
...
@Component({
  selector: 'mccs-fallback-mechanism',
  templateUrl: './fallback-mechanism.component.html',
  styleUrls: ['./fallback-mechanism.component.scss']
})
export class FallbackMechanismComponent implements OnInit {
  ...
  constructor(
    ...
    public fallbackService: FallbackMechanismService,
    ...
    this.duplicateBusinessError = this.translate.instant('communication.fallback.ValidationError.DuplicateBusiness');
    this.nullCheckError = this.translate.instant('communication.fallback.ValidationError.nullCheckError');
  ...
  ngOnInit(): void {
    ...
    this.fallbackHeader = [
      { header: this.translate.instant('communication.fallback.tabel.name'), field: 'name', width: '95%' },
      { header: this.translate.instant('communication.fallback.tabel.delete'), field: 'delete', fieldType: 'deleteButton', width: '5%'}
    ];

    this.fallupHeader = [
      {
        header: this.translate.instant('communication.fallback.tabel.FollowupEventName'),
        field: 'followUpEventName.caption',
        property: 'followUpEventName',
        width: '50%'
      },
      {
        header: this.translate.instant('communication.fallback.tabel.CommunicationMedium'),
        field: 'communicationMedium.caption',
        property: 'communicationMedium',
        width: '25%'
      },
      { header: this.translate.instant('communication.fallback.tabel.Seqnr'), field: 'seqNr', property: 'seqNr', width: '20%' },
      { header: this.translate.instant('communication.fallback.tabel.delete1'), field: 'delete', property: 'deleteButton', width: '5%', pSortableColumnDisabled: true }
    ];
  }
  ...
  buildConfiguration() {
    ...
    nameError.validationMessage =
      this.translate.instant('communication.fallback.ValidationError.name') +
      this.translate.instant('communication.fallback.ValidationError.required');
    ...
    maxValValidation.validationMessage = this.translate.instant('communication.fallback.ValidationError.numberInt32Check');
    ...
    SeqNrError.validationMessage =
      this.translate.instant('communication.fallback.ValidationError.Seqnr') +
      this.translate.instant('communication.fallback.ValidationError.required');
    this.RequiredSeqNr.required = true;
    this.RequiredSeqNr.Errors = [SeqNrError];
    this.RequiredSeqNr.maxValueValidation = this.translate.instant('communication.fallback.ValidationError.InputIncorrect');
